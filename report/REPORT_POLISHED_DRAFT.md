...
Appendix E. Optional W&B Logging
...
| Optional W&B dashboard | Metrics, images, tables, and artifact bundles derived from local run directories | Appendix E |
...
W&B support is optional. It is useful for browsing experiments, but the final report should still cite local artifacts generated by the benchmark pipeline.

The logging convention is:

```text
project = distributed-mpot-course
group   = experiment label, for example final_macbook_air_2d
name    = local run id, for example mpi-final_macbook_air_2d-N824-P4
